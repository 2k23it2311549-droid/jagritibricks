...
export default function ProductManagement() {
    ...
    const handleImageChange = async (e) => {
        // Image upload disabled for now as Supabase Storage is removed
        alert("Image upload is currently disabled. Please enter an Image URL manually.")
        /*
        try {
    ...
    return (
        <AdminLayout title="Product Management">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                ...
                <button
                    onClick={() => openModal()}
                    className="dora-floating-fab bg-gradient-to-r from-brand-orange to-red-600 text-white px-6 py-3 rounded-full font-bold shadow-lg flex items-center gap-2 group"
                >
                    <span className="text-xl group-hover:rotate-90 transition-transform duration-300">+</span> Add Product
                </button>
            </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {filteredProducts.map((product) => (
                            <tr key={product.id} className="dora-table-row hover:bg-gray-50 transition-colors">
            ...
            {/* Modal */}
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm overflow-y-auto h-full w-full flex items-center justify-center z-50 animate-fadeIn">
                    <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-lg mx-4 dora-modal-pop border border-white/20">
                        <h2 className="text-2xl font-bold mb-6 text-gray-800 border-b pb-2">{currentProduct ? 'Edit Product' : 'Add New Product'}</h2>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                ...
                                <input name="image_url" value={formData.image_url} onChange={handleInputChange} className="mt-1 w-full border rounded p-2" placeholder="https://example.com/image.jpg" />
                                <div className="mt-2 text-xs text-gray-500">Uploads disabled. Enter a direct image URL.</div>
                                {/* 
                                <div className="mt-1 flex items-center space-x-4">
                                    ...
                                    <input
                                        ...
                                        disabled={uploading}
                                    />
                                    {uploading && <span className="text-sm text-brand-orange animate-pulse">Uploading...</span>}
                                </div>
                                */}
                            </div>

                            <div className="flex justify-end space-x-3 mt-6 pt-4 border-t">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 border rounded-lg text-gray-600 hover:bg-gray-100 transition">Cancel</button>
                                <button type="submit" disabled={uploading} className="px-6 py-2 bg-gradient-to-r from-brand-orange to-red-600 text-white rounded-lg hover:shadow-lg disabled:opacity-50 transition-all duration-300 transform hover:scale-105">Save Product</button>
                            </div>
                        </form>
                    </div>
                </div>
