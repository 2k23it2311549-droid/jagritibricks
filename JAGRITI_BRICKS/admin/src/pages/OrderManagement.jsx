...
export default function OrderManagement() {
    ...
    const handleDateUpdate = async (orderId, date) => {
        try {
            await api.put(`/admin/orders/${orderId}`, { delivery_date: date })
            fetchOrders()
        ...
        try {
            await Promise.all(
                selectedOrders.map(id =>
                    api.put(`/admin/orders/${id}`, { status: newStatus })
                )
            )
            fetchOrders()
            setSelectedOrders([])
    ...
    const filteredOrders = orders.filter(order => {
        // Status filtering is handled by API now, but we keep this for search
        // and in case 'all' fetches everything
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex items-center space-x-3 w-full md:w-auto overflow-x-auto">
                        ...
                        <button
                            ...
                            className="dora-floating-fab flex items-center px-4 py-2 bg-gradient-to-r from-brand-orange to-red-600 text-white rounded-full hover:shadow-lg transition font-medium whitespace-nowrap"
                        >
                            <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
